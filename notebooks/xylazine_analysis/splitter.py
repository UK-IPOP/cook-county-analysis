...
df[df["case"] == 0].dropna().to_csv("fentanyl_controls.csv", index=False)


df = pd.read_csv("./xylazine_matched_stimulant.csv")
df[df["case"] == 1].dropna().to_csv("stimulant_cases.csv", index=False)
df[df["case"] == 0].dropna().to_csv("stimulant_controls.csv", index=False)
