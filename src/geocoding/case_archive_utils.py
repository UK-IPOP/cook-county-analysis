...
def geocode_case_archive(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['coded_lat'] = df['geo_location'].apply(lambda x: x.latitude if pd.notna(x) else None)
    df['coded_long'] = df['geo_location'].apply(lambda x: x.longitude if pd.notna(x) else None)
    df['coded_score'] = df['geo_location'].apply(lambda x: x.raw.get('score') if pd.notna(x) else None)
