...
if __name__ == "__main__":
    if FAST:
        ...
        # loaded = loaded.loc[:5_000]
        print(
            f"Geocoding case archive data... there are {int(len(df) / 500)} bunches..."
        )
        for bunch in trange(0, len(df) - 500, 500):
            asyncio.run(au.case_archive_runner(df.loc[bunch : bunch + 500]))
